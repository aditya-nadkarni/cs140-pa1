...
/*
 * File:     mv_mult_test_mpi.c
 *
 * Purpose:  test matrix vector multiplication y=Ax.
 * Matrix A is a square matrix of size nxn. Column vectors x and y are of size nx1
 * Input test matrix:  A[i][j]=c in all positions.  y[i] is 0 in all positions
 *		  x[i]= i for 0<=i<n
 * For simplicity, we assume n is divisible by no_proc
 */
...
int itmv_mult (double local_A[] /* in */ ,
               double local_x[] /* in */ ,
               double local_d[] /* in */ ,
               double local_y[] /* out */ ,
               double global_x[] /* out */ ,
...
void
print_error (char *msgheader, char *msg)
{
  if (my_rank == 0) {
...
void
print_itmv_sample_distributed (char *msgheader, double local_A[],
                               ...
                               int t, int blocksize)
{
  ...
  for (i = 0; i < n; i++) {
    if (procmap (i, blocksize) == my_rank) {
      ...
      printf
        ("%s Proc %d x[%d] locally x[%d] = %f; d[%d] locally d[%d] = %f\n",
...
/*-------------------------------------------------------------------------------
 * Test if the  t iterations of parallel computation {y=Ax;  x=y} matches the expectation.
 ...
 * 	matrix_type: 0 means regular matrix. 1 (UPPER_TRIANGULAR) means upper triangular.
 ...
 */
double *
compute_expected (char *testmsg, int n, int t, int matrix_type)
{
  int i,
    j,
    start;
...
char *
validate_vect (char *msgheader, double global_x[], int n, int t,
               int matrix_type)
{
  int i;
  double *expected;
 ...
 * In args:
 *   	*local_A is the starting address of local space for matrix A with rowwise block mapping
 *   	*local_x is the starting address of local space for vector x with rowwise block mapping
 *   	*local_d is the starting address of local space for vector d with rowwise block mapping
 *   	*local_y is the starting address of local space for vector y with rowwise block mapping
 *   	*global_x is the starting address of space for vector x with size n
 * 	blocksize is the size of local array
 * 	n is the number of columns (and rows)
 */

int
allocate_space (double **local_A, double **local_x, double **local_d,
 ...
 *   matrix  A of size nxn:  A[i,i]=0 for diagnal elements. A[i,j]=-1/n for non-diagonal elements
 * When matrix_type is 	  UPPER_TRIANGULAR
 *   matrix  A of size nxn:  A[i,i]=0 for diagnal elements. A[i,j]=-1/n for upper diagonal elements with i<j
 *				all lower triangular elements are 0
 * In args:
 *   	*local_A is the starting address of local space for matrix A with rowwise block mapping. # of rows=blocksize
 *   	*local_x is the starting address of local space for vector x with rowwise block mapping. # of elements=blocksize
 *   	*local_d is the starting address of local space for vector d with rowwise block mapping. # of elements=blocksize
 *   	*local_y is the starting address of local space for vector y with rowwise block mapping. # of elements=blocksize
 * 	blocksize is the size of local array
 * 	n is the number of columns (and rows)
 *	marix_type: matrix type UPP_TRIANGULAR  means A is upper triangular otherwise just reguar square matrix
 *      my_rank: my process ID (started from 0)
 * Return value: If failed, return FAIL (0)
 * 		If successful, return 1 (SUCC)
 * Global variable: This function should NOT use any global variable
 */
int
init_matrix (double *local_A, double *local_x, double *local_d,
             double *local_y, int blocksize, int n, int matrix_type,
             int my_rank)
{

  if (local_A == NULL || local_x == NULL || local_d == NULL || local_y == NULL
      || blocksize <= 0)
    return FAIL;
  ...
  if(matrix_type == UPPER_TRIANGULAR){
    ...
    /** NOT UPPER_TRIANGULAR **/
    // int init = my_rank*blocksize;
    // int upper_bound = blocksize*my_rank + blocksize;
    // printf("N: %d\n", n);
    // printf("blocksize: %d\n", blocksize);
    for(int i = 0; i < blocksize; i++){
      local_d[i] = (2.0*n - 1)/n;
 ...
 */
char *
itmv_test (char *testmsg, int test_correctness, int n, int matrix_type, int t)
{
  double startwtime = 0,
    endwtime = 0;
  double *local_A,
  ...
  succ =
    allocate_space (&local_A, &local_x, &local_d, &local_y, &global_x,
                    blocksize, n);
  ...
  succ =
    init_matrix (local_A, local_x, local_d, local_y, blocksize, n,
                 matrix_type, my_rank);
  ...
  succ =
    itmv_mult (local_A, local_x, local_d, local_y, global_x, matrix_type, n,
